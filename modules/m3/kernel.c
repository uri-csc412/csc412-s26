...
void kernel(const char* command) {
    ...
    // TODO (Part 1): (re-)Initialize the kernel pagetable with the correct
    // permissions using virtual_memory_map
...
// TODO (Part 2): Implement this function to use to allocate a pagetable.
// This will need to be called to allocate the first level pagetable,
// and then passes as the allocator function for calls to virtual_memory_map
// to allocate the 2nd, 3rd, and 4th level pagetables if necessary.
// static x86_64_pagetable* pagetable_alloc() {
//
// }
...
void process_setup(pid_t pid, int program_number) {
    ...
    // TODO (Part 2): Each process needs to have it's own pagetable that
    // contains a copy of the kernel memory mappings (addresses < PROC_START_ADDR)
    processes[pid].p_pagetable = kernel_pagetable;
    pageinfo[PAGENUMBER(kernel_pagetable)].refcount += 1;

    // Load the current process into memory (don't touch this)
    int r = program_load(&processes[pid], program_number, NULL);
    assert(r >= 0);

    // Allocate physical and virtual memory for the stack of the current process
...
void exception(x86_64_registers* reg) {
    ...
    case INT_SYS_PAGE_ALLOC: {
        ...
        // TODO (Part 1): Mhmm... addr could be a kernel address. Should a
        // usespace process be able to allocate a page of kernel memory?
