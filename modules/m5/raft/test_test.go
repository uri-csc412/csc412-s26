...
func TestInitialElection3A(t *testing.T) {
	...
	if term1 != term2 {
		t.Fatalf("warning: term changed from %d to %d even though there were no failures", term1, term2)
	}
...
// Crash the leader immediately after election and check if the system stabilizes with a new leader.
func TestRapidLeaderCrash3A(t *testing.T) {
	servers := 5
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()

	cfg.begin("Test (3A): rapid leader crash and recovery")
...
// Partition the cluster into two groups (one with majority, one minority), allow the majority to elect a leader, then heal the partition and check leader stability.
func TestPartitionHealing3A(t *testing.T) {
	servers := 5
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()

	cfg.begin("Test (3A): partition healing")

	// partition minority (2 nodes)
	cfg.disconnect(0)
	cfg.disconnect(1)

	// majority should elect a leader
	cfg.checkOneLeader()
...
// Ensures that when a leader is elected, all servers agree on the same leader.
func TestLeaderAgreement3A(t *testing.T) {
	servers := 3
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()

	cfg.begin("Test (3A): leader agreement")

	leader1 := cfg.checkOneLeader()

	time.Sleep(50 * time.Millisecond)

	// Ask each server who the leader is
	for i := 0; i < servers; i++ {
		if cfg.rafts[i] != nil {
			_, isLeader := cfg.rafts[i].GetState()
			if isLeader && i != leader1 {
				t.Fatalf("server %d thinks it is leader, but leader is %d", i, leader1)
			}
		}
	}
...
// Tests that a disconnected follower properly rejoins and syncs with the leader.
func TestFollowerRejoin3A(t *testing.T) {
	servers := 3
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()

	cfg.begin("Test (3A): follower rejoin")
	...
	// re-econnect the follower
	cfg.connect(follower)

	time.Sleep(RaftElectionTimeout)

	// ensure still only one leader
	cfg.checkOneLeader()
